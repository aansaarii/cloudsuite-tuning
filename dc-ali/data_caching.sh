...
ssh $SERVER_SSH_USER@$SERVER_ADDRESS -p $SERVER_SSH_PORT docker stop $SERVER_NAME
ssh $SERVER_SSH_USER@$SERVER_ADDRESS -p $SERVER_SSH_PORT docker rm $SERVER_NAME
ssh $SERVER_SSH_USER@$SERVER_ADDRESS -p $SERVER_SSH_PORT docker run -d --cpuset-cpus $SERVER_CPU_SET  --name $SERVER_NAME -p $SERVER_PORT:11211 -d $SERVER_IMAGE -t $SERVER_THREADS -m $SERVER_MEMORY -n $KEY_LENGTH
